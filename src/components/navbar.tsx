...
export default function Navbar(props: NavbarProps) {
	...
	return (
		<>
			<nav className="w-full h-24 relative z-10 items-center justify-between flex py-6 border-b-[0.1px] border-opacity-10 border-white">
				<div className="h-24 items-center justify-between flex">
					...
					{ShowNavLogoText ? (
						<div className="flex">
							<h3 className="inline-block px-1 mt-[0.45rem] text-[0.7rem] lg:text-[1rem] dark:text-white dark:opacity-10 font-jetbrains opacity-20">
								{"/*"}
							</h3>
							<Link
								className="font-jetbrains text-pllbllue dark:text-white text-[1.75rem] lg:text-[1.9rem] font-semibold inline-fold"
								to="/"
							>
								FusionSid
							</Link>
							<h3 className="inline-block px-1 mt-[1rem] text-[0.7rem] lg:text-[1rem] dark:text-white dark:opacity-10 font-jetbrains opacity-20">
								{"*/"}
							</h3>
						</div>
					) : null}
				...
				{!showMobileNav ? (
					<ul className="flex flex-row mr-12 my-24 lg:gap-16 md:gap-8 justify-center">
						...
						<li
							...
							className={
								"whitespace-nowrap group " + underlineTransition
							}
						>
					...
					<>
						...
						<div
							className={`top-0 right-0 w-full dark:bg-pgrey bg-[#9a9787] p-5 text-white fixed h-full z-40 ease-in-out duration-300 ${
								navbarOpen
									? "translate-x-0 "
									: "translate-x-full"
							}`}
							...
							<ul className="flex flex-col m-28 items-center">
								{props.navlinks.map((navItem) => (
									...
									>
										<Link
											className={
												"text-[2rem] py-5 font-jetbrains inline-block text-black dark:text-white " +
												underlineTransition +
								...
								>
									<Link
										className={
											"text-black dark:text-white py-3 text-[2rem] inline-block font-jetbrains " +
											underlineTransition +
											" transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300"
										}
										to="/terminal"
